...
describe('automateOpenPlatformSetup', () => {
  ...
  interface SetupMockOptions {
    ...
    /** null 模拟 create 成功但未返回 versionId。 */
  ...
  function subscriptionFetchImpl(
    ...
    return (async (url: string | URL | Request, init?: RequestInit) => {
      ...
      if (href.includes('/app_version/create/')) {
        const versionId = options.versionId === undefined ? 'v1' : options.versionId;
        return Response.json({ code: 0, data: versionId ? { versionId } : {} });
      }
      if (options.failPublish && href.includes('/publish/commit/')) {
        return Response.json({ code: 1, msg: 'publish rejected' });
      }
      return sub.handle(href, init) ?? Response.json({ code: 0 });
    }) as typeof fetch;
  }
  ...
  async function runSetupWithMock(
    ...
    return automateOpenPlatformSetup({
      appId: 'cli_x',
      sessionFilePath: sessionFile,
      fetchImpl: subscriptionFetchImpl(sub, calls, options),
      scopeManifest: { scopes: { tenant: ['im:message'], user: [] } },
      requireVcMeetingEvents: options.requireVcMeetingEvents,
    });
  }
  ...
  it('requires VC events in their expected app/user identity buckets for a listener', async () => {
    const vcEvents = [...VC_MEETING_APP_EVENTS, ...VC_MEETING_USER_EVENTS];
    const sub = openPlatformSubscriptionMock('cli_x', {
      rejectEventNames: vcEvents,
      initial: {
        appEvents: [...BOT_BASELINE_APP_EVENTS, ...VC_MEETING_USER_EVENTS],
        userEvents: [...VC_MEETING_APP_EVENTS],
        callbacks: [...BOT_BASELINE_CALLBACKS],
        callbackMode: 4,
      },
    });
    const calls: string[] = [];
    const result = await runSetupWithMock('botmux-sub-vc-identity-', sub, calls, {
      requireVcMeetingEvents: true,
    });

    expect(result).toMatchObject({ ok: false, reason: 'api_error' });
    if (!result.ok) {
      expect(result.message).toContain('VC 事件未按正确身份订阅');
      expect(result.missingVcEvents).toEqual(vcEvents);
    }
    expect(calls.some(u => u.includes('/publish/commit/'))).toBe(false);
  });

  it('requires a created version id before publishing listener event changes', async () => {
    const sub = openPlatformSubscriptionMock('cli_x', {
      initial: {
        appEvents: [...BOT_BASELINE_APP_EVENTS, ...VC_MEETING_APP_EVENTS],
    ...
    const calls: string[] = [];
    const result = await runSetupWithMock('botmux-sub-vc-version-', sub, calls, {
      requireVcMeetingEvents: true,
      versionId: null,
    });
  ...
  it('fails the listener setup when publishing the event-ready version fails', async () => {
    const sub = openPlatformSubscriptionMock('cli_x', {
      initial: {
        appEvents: [...BOT_BASELINE_APP_EVENTS, ...VC_MEETING_APP_EVENTS],
        userEvents: [...VC_MEETING_USER_EVENTS],
    ...
    const calls: string[] = [];
    const result = await runSetupWithMock('botmux-sub-vc-publish-', sub, calls, {
      requireVcMeetingEvents: true,
      failPublish: true,
    });

    expect(result).toMatchObject({ ok: false, reason: 'api_error' });
    if (!result.ok) expect(result.message).toContain('publish rejected');
    expect(calls.some(u => u.includes('/publish/commit/'))).toBe(true);
  });
